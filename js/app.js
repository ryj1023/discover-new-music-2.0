...
var getMusic = function(tags) {
  $.post("/get-bands-list", { tags: tags.toLowerCase() }).done(res => {
    getMusicList(res);
  });
};
...
var showSearchResults = function(resultNum, resultName) {
  if ($("input[name='query']").val() == "") {
    var results =
      resultNum + " results for <strong> " + resultName + " </strong>";
  } else {
    var results =
      resultNum + " results for <strong> " + $("input[name='query']").val();
    +" </strong>";
  }
  return results;
};
  ...
  $(".article").each(function() {
    ...
    if (content.length > showChar) {
      ...
      var html =
        ...
        h +
        '</span>&nbsp;&nbsp;<a href="" class="morelink"> <p style="background-color: #1976D2; margin-right: 84%;">' +
        moretext +
        "</p></a></span>";
      $(this).html(html);
    }
...
var initMap = function(myLatLng, events) {
  // AIzaSyBri9B_6AUmBjkKnzV18eb--zerOISaTHY
  myLatLng = {
    lat: events
      ? parseFloat(events[0].venue.latitude)
      : parseFloat(myLatLng.latitude),
    lng: events
      ? parseFloat(events[0].venue.longitude)
      : parseFloat(myLatLng.longitude)
  };
